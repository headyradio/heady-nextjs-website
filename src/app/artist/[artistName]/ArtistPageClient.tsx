...
export default function ArtistPageClient({ artistName }: ArtistPageClientProps) {
          ...
          <div className="divide-y divide-white/10">
            {artistDetails.songs?.map((song: any, index: number) => (
              <div key={song.id} className="flex items-center gap-4 p-4 hover:bg-white/5 transition-colors group">
                <Link 
                  href={songUrl(artistDetails.artist, song.title)}
                ...
                <div className="flex-grow min-w-0">
                  <Link href={songUrl(artistDetails.artist, song.title)}>
